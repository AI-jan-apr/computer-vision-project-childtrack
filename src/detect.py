...
# the new code with results only for detect and draw the bounding box: 

# ============================================================
# detect.py — Model Loading & Visualization
# ============================================================
# Owner: Person 1 (CV Engineer - Detection)
#
# What this file does:
#   1. Load the trained YOLO model (best.pt)
#   2. Draw bounding boxes and labels on the frame
#
# Note:
#   Detection + Tracking are now both handled in track.py
#   using model.track() — so this file only loads the model
#   and handles visualization.
#
...
def load_model(model_path=YOLO_MODEL_PATH):
    ...
    Called once at startup in main.py or track.py.
...
# ──────────────────────────────────────
# 2. Draw tracks on frame (for visualization)
# ──────────────────────────────────────
def draw_detections(frame, tracks):
    """
    Draws bounding boxes and labels on the frame.
    Receives the output from track.py (track_persons).
    ...
    Args:
        ...
        tracks — list of dicts from track_persons() in track.py
                 each dict has: bbox, label, conf, track_id, direction
    ...
    for t in tracks:
        ...
        # Label + confidence + ID above the box
        text = f"ID:{track_id} | {label} {conf}"
...
# ──────────────────────────────────────
# 3. Quick test — run this file directly
#    to verify model loads correctly
# ──────────────────────────────────────
if __name__ == "__main__":
    ...
    Just checks that the model loads and prints class names.
    For full detection + tracking test, run:
        python src/track.py
    """
    model = load_model()
    print(f"Classes: {model.names}")
    print("Model is ready. Run track.py for full test.")
...
# ==================The old code with results oll info =========================
# # ============================================================
